...
export default {
  ...
  methods: {
    phoneBlur() {
			if (!checkPhone(this.seckillForm.serviceWhchat)) {
				this.$message({
					message: '电话号码格式错误',
					position: 'bottom',
					type: 'error'
    ...
    async getCode(){
      ...
      http.get('/login/official/website/customer/joinsendcode', params)
          .then(res =>{
            console.log(res);
          })
          .catch(err =>{
    ...
    async postCode(code){
      ...
      http.postJson('/login/official/website/customer/joinvailcode', JSON.stringify(params))
          .then(res =>{
            console.log(res);
          })
          .catch(err => {
